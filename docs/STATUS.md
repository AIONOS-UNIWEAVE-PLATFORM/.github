...
| #98 | IntelliRAG — Query integration (MCP server) | Himanshu | Mar 10 | **11/11** | Checklist complete. v0.1 built. Review Monday — likely ready to move to Done. |
| #118 | UniScript — voice AI prompt engineering service | Agam | — | **9/10 features** | **149 tests passing.** Complete: one-shot generation, chat/iterative refinement, packs system, prompt assembler, API key auth, credits system, AI Gateway integration (llm-core), product telemetry (OpenTelemetry), MCP server (10 tools, dual transport). **Remaining: Portal — admin + developer views, templates CRUD.** |
...
| #108 | WABA Provisioning | Ravinder (not on Trello) | Mar 10 | Agam coordinating with Ravinder. Blocks #95. **3 days to deadline.** |
...
These were surfaced during the full 13-repo architecture deep-dive (2026-03-06). **Not yet communicated to engineering — escalate Monday (Day 3+).**
...
- **UniScript**: 9/10 core features built. FastAPI + PostgreSQL + Redis. AI Gateway integrated via typed Python client (llm-core). MCP server with 10 tools, dual transport (HTTP + STDIO). Product telemetry with OpenTelemetry.
